...
1. Off camera, `setup.sh` creates `~/bg-demo` (or `$BRAINGATE_DEMO_DIR`): a one-file repository
   ...
   It also clears the previous recording's thread and goal (`/forget`, `/new`) so every take starts
   fresh.
2. On camera: a question, the plan, `/why`, a second opinion from Grok on the same goal, then
   `/auto` and the fix applied in the workspace by whichever worker BrainGate routes it to.

It spends a few small requests on the providers involved, and needs Grok Build signed in; to use
Codex instead, change `/use grok` to `/use codex` in the tape. A step fails when the provider it
needs is at its limit (`/why` shows `quota-exhausted`); wait for the limit to reset, then record
again.
...
- **`last value was: >`**: BrainGate refused the request and returned to the prompt, so the plan
  question never came. The lines VHS printed before the failure show which step it was. Run
  `cd ~/bg-demo && braingate` yourself, repeat that step, and read the refusal.
