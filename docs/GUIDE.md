# BrainGate — the full guide

This is the complete reference. For the short version, start with the [README](../README.md).

**One local control plane for the AI coding subscriptions you already use.**

BrainGate is a technical preview (v0.1.0) for coordinating official AI coding CLIs across multiple software projects while keeping project context isolated, controlling quota usage, and recording what every agent did.
...
[العربية](i18n/README.ar.md) is current as of this release; the others predate the current
...
(ADR [0021](adr/0021-big-writes-and-default-profiles.md)).
...
ADR [0014](adr/0014-native-runtime-preservation.md) states the principle and classifies every
...
- **A refusal is a stop, not a puzzle to route around.** When a provider CLI refuses, rate-limits, or
  ...
  (ADR [0012](adr/0012-quota-state-is-native-only.md)) — it never retries past the refusal,
...
[`docs/PROVIDER_POLICY_AUDIT.md`](PROVIDER_POLICY_AUDIT.md) for exactly how each supported CLI
is invoked today.
...
(ADR [0012](adr/0012-quota-state-is-native-only.md)).
...
See [`docs/adr/0008-operator-accepted-providers.md`](adr/0008-operator-accepted-providers.md)
and [`docs/adr/0009-grok-sandbox-is-provable.md`](adr/0009-grok-sandbox-is-provable.md).
...
[0021](adr/0021-big-writes-and-default-profiles.md)); reach for this only in a script, in
...
or touching a provider profile. See [`docs/DOGFOOD.md`](DOGFOOD.md).
...
| [`docs/SECURITY.md`](SECURITY.md) | the security boundaries and why they hold |
| [`docs/SAFE_EXECUTION.md`](SAFE_EXECUTION.md) | worktrees, command allowlists, fail-closed rules |
| [`docs/ROUTING_AND_REVIEW.md`](ROUTING_AND_REVIEW.md) | how a task is classified and routed |
| [`docs/DOGFOOD.md`](DOGFOOD.md) | trialling BrainGate on a real repository |
| [`docs/PROVIDER_DISCOVERY.md`](PROVIDER_DISCOVERY.md) | what is probed per provider, and why fields stay `unknown` |
| [`docs/MEMORY_AND_CONTEXT.md`](MEMORY_AND_CONTEXT.md) | the memory model and its single validated write path |
...
- [`docs/OPEN_SOURCE_AND_COMMERCIAL.md`](OPEN_SOURCE_AND_COMMERCIAL.md) — intended OSS/commercial boundary, business model, pricing hypotheses, architecture seams, and moat thesis.
- [`docs/PUBLIC_RELEASE_CHECKLIST.md`](PUBLIC_RELEASE_CHECKLIST.md) — concrete prerequisites before making the project public or charging for cloud/team features.
- [`docs/PRODUCT.md`](PRODUCT.md) — product definition and business principles.
...
See [`docs/DOGFOOD.md`](DOGFOOD.md) for the real-project trial workflow.
