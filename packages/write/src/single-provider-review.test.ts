...
function router(): CapabilityRouter {
  ...
  const runtime = { available: true, quotaState: "healthy" as const, quotaHint: 0.1, refusalBackoffUntil: null, quotaObservedAt: null, observedAt: "2026-09-07T00:00:00.000Z" };
  registry.register({
...
test("real write plan can fall back to the same model in a fresh invocation when it is the only reviewer", () => {
  ...
  registry.register({
    ...
    writeCapable: true, reasoning: 96, underlyingFamily: null,
  }, { available: true, quotaState: "healthy", quotaHint: 0.1, refusalBackoffUntil: null, quotaObservedAt: null, observedAt: "2026-09-07T00:00:00.000Z" });
  const classification = classifyTask({ text: "change the button label", mode: "write" });
  const plan = buildWriteTaskPlan({
