...
test("network access is the operator's decision, never an attestation's", () => {
  const planner = grant({ role: "planner" });
  assert.equal(grants(planner, "web"), false);
  assert.match(planner.refused.find((item) => item.capability === "web")!.reason, /recorded acceptance/);
  assert.ok(grants(grant({ role: "planner", operatorAccepted: true }), "web"));
});
...
test("the grant can only take a guarantee away, never invent one", () => {
  ...
  const withWeb = guaranteesFor(grant({ role: "planner", operatorAccepted: true }), honest);
...
test("a run that needs a capability it was refused fails with the reason, not a generic block", () => {
  assert.throws(
    ...
    (error: unknown) => error instanceof BrainGateInvariantError && error.code === "SHADOW_GRANT_INSUFFICIENT" && /recorded acceptance/.test(error.message),
  );
});
